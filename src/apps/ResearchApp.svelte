<script>
  ...
  $: activePath = normalizedSubroute ? `/research/${normalizedSubroute}` : '/research';
  $: content = resolveContent('research', normalizedSubroute);
</script>
...
<div class="app-layout" data-sidebar-collapsed={sidebarCollapsed}>
  {#if !sidebarCollapsed}
    <aside class="app-sidebar">
      <h3>Research</h3>
      <p class="app-intro">Research themes, groups, and member areas in the Computer Science Division.</p>
      <nav aria-label="Research sections">
        {#each APP_NAV_LINKS.research as section}
